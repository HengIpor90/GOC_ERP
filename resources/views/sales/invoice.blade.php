...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .payment-wrap { display: flex; justify-content: flex-start; margin-top: 15px; }
        .qr-card {
            width: 300px;
            padding: 10px 8px;
        ...
        .footer { display: grid; grid-template-columns: 1fr auto; gap: 25px; align-items: end; margin-top: 30px; padding-top: 22px; border-top: 1px solid var(--line); }
...
<body>
    ...
    <main class="sheet">
        <header class="invoice-head">
            <div class="brand">
                ...
                <span>
                    ...
                    <small>ផ្តត់ផ្គង់គ្រឿងសំណង់និងអគ្គិសនី គ្រប់ប្រភេទ</small>
                </span>
            </div>
        ...
        <section class="meta-grid">
            ...
            <div class="meta-block">
                ...
                <dl class="invoice-meta">
                    ...
                    <dt>Date</dt>
                    <dd>{{ $sale->sold_at->timezone('Asia/Phnom_Penh')->format('F d, Y') }}</dd>
                    <dt>Time (Cambodia)</dt>
                    <dd>{{ $sale->sold_at?->timezone('Asia/Phnom_Penh')->format('h:i A') }}</dd>
                    <dt>Cashier</dt>
                    <dd>{{ $sale->creator->name ?? 'System' }}</dd>
                    <dt>Payment</dt>
                    <dd>{{ strtoupper($sale->payment_method ?? 'ABA') }}</dd>
                    <dt>Status</dt>
                    <dd><span class="paid">Paid / Completed</span></dd>
                </dl>
            </div>
        </section>
        ...
        @php($saleSubtotal = (float) $sale->items->sum(fn ($item) => $item->subtotal ?? $item->total))
        <section class="totals">
            <div class="total-row"><span>Subtotal</span><strong>${{ number_format($saleSubtotal, 2) }}</strong></div>
            <div class="total-row"><span>Discount</span><strong>−${{ number_format(max(0, $saleSubtotal - (float) $sale->total), 2) }}</strong></div>
            <div class="total-row"><span>Tax</span><strong>$0.00</strong></div>
            <div class="total-row grand-total"><span>Total</span><strong>${{ number_format((float) $sale->total, 2) }}</strong></div>
        </section>
        ...
        <footer class="footer">
            <div>
                ...
                <p>This invoice was generated by {{ config('app.name', 'OMNIHUB') }}. Please keep it for your records.</p>
            </div>
